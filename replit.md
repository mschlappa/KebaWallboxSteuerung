...
The backend tracks cable status changes via UDP broadcast listener (`wallbox-broadcast-listener.ts`) which handles Input X1, Plug, State, and E pres broadcasts in real-time. Plug status changes are persisted with timestamps for lückenlose tracking.
...
- **Settings Page Organization**: Logical grouping and positioning of critical settings with Accordions for advanced parameters.
- **Broadcast-Handler Architecture**: Real-time processing of Wallbox UDP broadcasts for Input X1 (potenzialfreier Kontakt), Plug (cable status), State (operational state), and E pres (energy present). Input broadcasts trigger strategy changes with controller integration and persistence. Plug broadcasts use in-memory sentinel with timestamp-based change detection and persistence.
...
- **Potenzialfreier Kontakt (X1) Integration**: Configurable strategy selection for closed X1 contact via UDP broadcast handler. Supports all 4 charging strategies plus "Off". Real-time strategy changes without polling delay. Ideal for external SmartHome systems, timers, or energy managers.
